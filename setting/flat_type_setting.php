<?php 
include('../header.php');
include(ROOT_PATH.'language/'.$lang_code_global.'/lang_flat_setting.php');
if(!isset($_SESSION['objLogin'])){
	...
	$form_url = WEB_URL . "setting/flat_type_setting.php";
...
if(isset($_POST['txtFacilityName'])){
    ...
    //die;
                    ...
                    if($_POST['hdnSpid'] == '0'){
                    $year = date('Y');
                    echo $sql="INSERT INTO `flat_type_master`(`flatType`,`flatArea`,`status`,`cDate`,c_userid,branch_id) VALUES ('$_POST[txtFacilityName]','$_POST[txtFacilityFor]','$_POST[txtFacilityStatus]','".$date."','".(int)$_SESSION['objLogin']['user_id']."','" . $_SESSION['objLogin']['branch_id'] . "')";	
                    //die;
                    mysql_query($sql, $link);
                    mysql_close($link);
		    $url = WEB_URL . 'setting/flat_type_setting.php?m=add';
		    header("Location: $url");
		}
else{
	date_default_timezone_set("Asia/Kolkata");
...
//    echo '<pre>';
//    die;
	$sql_update="UPDATE `flat_type_master` set "
                . "flatType ='".$_POST['txtFacilityName']."',"
                . "`flatArea`='".$_POST['txtFacilityFor']."',"
                . "`status`='".$_POST['txtFacilityStatus']."',"
                . "`cDate`='".$date."' where flatId = '".$_GET['id']."'";	
			mysql_query($sql_update, $link);
			mysql_close($link);
                        $url = WEB_URL . 'setting/flat_type_setting.php?m=up';
                        header("Location: $url");
...
if(isset($_GET['spid']) && $_GET['spid'] != ''){
		$result_location = mysql_query("SELECT * FROM flat_type_master where flatId= '" . (int)$_GET['spid'] . "'  ",$link);
...
//                    die;
		 	$f_name = $row['flatType'];
			$f_event = $row['flatArea'];
			$f_status = $row['status'];
			//$issue_date = $row['issue_date'];
			$button_text = $_data['update_button_text'];
			$form_url = WEB_URL . "setting/flat_type_setting.php?id=".$_GET['spid'];
			$hval = $row['flatId'];
		}
        ...
        <div class="box-body">
		 <div class="row">
          ...
          <div class="form-group col-md-4 col-xs-12">
            ...
            <input type="text" onkeypress="return isChar(event)" name="txtFacilityFor" value="<?php echo $f_event;?>" id="txtFacilityFor" class="form-control" />
...
<!--          <div class="form-group">
          ...
          <div class="form-group pull-right">
			...
			&nbsp;
            <input type="reset" onClick="javascript:window.location.href='<?php echo WEB_URL; ?>setting/flat_type_setting.php';" name="btnReset" id="btnReset" value="<?php echo $_data['reset'];?>" class="btn btn-primary"/>
          </div>
        </div>
...
$msg = "";
 if(isset($_GET['delid']) && $_GET['delid'] != '' && $_GET['delid'] > 0){
		$sqlx= "DELETE FROM `flat_type_master` WHERE flatId = ".$_GET['delid'];
...
<!--                    <th><?php echo $_data['text_8'];?></th>-->
                ...
                <tbody>
                  <?php
                                 $queryAllFacility = "SELECT * FROM flat_type_master where c_userid = '".(int)$_SESSION['objLogin']['user_id']."' AND branch_id = '" . $_SESSION['objLogin']['branch_id'] . "' ";
				...
				while($row = mysql_fetch_assoc($result)){
                  ...
                  <tr>
                    ...
                    <td><a class="btn btn-success" data-toggle="tooltip" href="javascript:;" onclick="$('#employee_view_<?php echo $row['flatId']; ?>').modal('show');" data-original-title="<?php echo $_data['view_text'];?>"><i class="fa fa-eye"></i></a> <a class="btn btn-primary" data-toggle="tooltip" href="<?php echo WEB_URL;?>setting/flat_type_setting.php?spid=<?php echo $row['flatId']; ?>" data-original-title="<?php echo $_data['edit_text'];?>"><i class="fa fa-pencil"></i></a> <a class="btn btn-danger" data-toggle="tooltip" onclick="deleteEmployeeSalary(<?php echo $row['flatId']; ?>);" href="javascript:;" data-original-title="<?php echo $_data['delete_text'];?>"><i class="fa fa-trash-o"></i></a>
                      <div id="employee_view_<?php echo $row['flatId']; ?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
...
<!--                                    <b><?php echo $_data['text_13'];?> :</b> <?php echo $row['ownerName']; ?><br/>-->
                                    <b><?php echo $_data['text_3'];?> :</b> <?php echo $row['flatType']; ?><br/>
                                    <b><?php echo $_data['text_6'];?> :</b> <?php echo $row['flatArea']; ?><br/>
...
<script type="text/javascript">
  function deleteEmployeeSalary(Id){
	...
	if(iAnswer){
		window.location = '<?php echo WEB_URL;?>setting/flat_type_setting.php?delid=' + Id;
	}
  }
  </script>
