...
if(isset($_GET['id']) && $_GET['id'] != '' && $_GET['id'] > 0){
	$sqlx= "DELETE FROM `tbl_add_complain` WHERE id = ".$_GET['id'];
        ...
        <table class="table sakotable table-bordered table-striped dt-responsive">
          <thead>
            ...
            <th><?php echo $_data['text_7'];?></th>
              ...
              <th><?php echo 'Resolve Period';?></th>
              <th><?php echo $_data['action_text'];?></th>
            </tr>
          </thead>
          <tbody>
            <?php
                                echo $queryAllComplain = "SELECT *,DATE_FORMAT(insertDate,'%D %M, %Y') as date,(
                                    CASE WHEN (CURDATE()) != (DATE_FORMAT(insertDate,'%Y-%m-%d'))
                                     THEN            CONCAT(FLOOR(HOUR(TIMEDIFF(resolveDate, insertDate)) / 24), ' Days ago')
                                        WHEN (CURDATE()) = (DATE_FORMAT(insertDate,'%Y-%m-%d'))        
                                     THEN          CONCAT(MOD(HOUR(TIMEDIFF(resolveDate, insertDate)), 24), ' Hr ago')
                                            WHEN (CURDATE()) = (DATE_FORMAT(insertDate,'%Y-%m-%d') AND (MOD(HOUR(TIMEDIFF(resolveDate, insertDate)), 24))=0)    
                                         THEN       CONCAT(MINUTE(TIMEDIFF(resolveDate, insertDate)), ' M')
                                                END
                                             )  AS days"
                                        . " FROM tbl_add_complain where branch_id = " . (int)$_SESSION['objLogin']['branch_id'] . " order by id";
				//die;
                                $result = mysql_query($queryAllComplain,$link);
...
//            echo '<pre>';
            ...
            <tr>
              ...
              <td><a class="btn btn-success" data-toggle="tooltip" href="javascript:;" onClick="$('#nurse_view_<?php echo $row['id']; ?>').modal('show');" data-original-title="<?php echo $_data['view_text'];?>"><i class="fa fa-eye"></i></a> <a class="btn btn-primary" data-toggle="tooltip" href="<?php echo WEB_URL;?>complain/addcomplain.php?id=<?php echo $row['id']; ?>" data-original-title="<?php echo $_data['edit_text'];?>"><i class="fa fa-pencil"></i></a> <a class="btn btn-danger" data-toggle="tooltip" onClick="deleteComplain(<?php echo $row['id']; ?>);" href="javascript:;" data-original-title="<?php echo $_data['delete_text'];?>"><i class="fa fa-trash-o"></i></a>
                  ...
                  <div class="modal-dialog">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        ...
                        <div class="row">
                          <div class="col-xs-12"> <b><?php echo $_data['text_7'];?> :</b> <?php echo $row['date']; ?><br/>
                            <b><?php echo $_data['text_5'];?> :</b> <?php echo $row['c_title']; ?><br/>
                            <b><?php echo $_data['text_6'];?> :</b> <?php echo $row['complain']; ?><br/>
                            <b><?php echo $_data['text_11'];?> :</b> <?php echo $row['days']; ?><br/>
