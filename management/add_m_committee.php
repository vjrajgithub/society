...
include(ROOT_PATH.'utility/common.php');
...
//     echo '<pre>';
	...
	echo $sql = "INSERT INTO tbl_add_management_committee(mc_name,member_type,mc_joining_date,mc_ending_date,mc_status,branch_id) values('$_POST[txtMCName]','$_POST[ddlMemberType]','$_POST[txtMCJoiningDate]','$_POST[txtMCEndingDate]','$_POST[txtFacilityStatus]','" . $_SESSION['objLogin']['branch_id'] . "')";
...
<!--          <div class="form-group">
          ...
          <div class="form-group col-md-4 col-xs-12">
            <label for="ddlMemberType"><?php echo $_data['add_new_form_field_text_1'];?> :</label>
            <select name="txtMCName" id="txtMCName" class="form-control">
              <option value="">--<?php echo 'Select Owner';?>--</option>
              <?php 
				  	$result_type = mysql_query("SELECT * FROM tbl_add_owner order by memberId ASC",$link);
					while($row_type = mysql_fetch_array($result_type)){
				  ?>
              <option <?php if($member_type == $row_type['memberId']){echo 'selected';}?> value="<?php echo $row_type['memberId'];?>"><?php echo $row_type['firstName'].$row_type['middleName'].$row_type['lastName'];?></option>
...
<!--          <div class="form-group">
          ...
          <div class="form-group col-md-4 col-xs-12">
            ...
            <select name="ddlMemberType" id="ddlMemberType" class="form-control">
              ...
              <?php 
				  	$result_type = mysql_query("SELECT * FROM tbl_add_member_type order by member_type ASC",$link);
					while($row_type = mysql_fetch_array($result_type)){
				  ?>
...
<!--            <input type="text" name="txtEmpAmount" value="<?php echo $amount;?>" id="txtEmpAmount" class="form-control" />-->
            <select name="txtFacilityStatus" id="txtFacilityStatus" class="form-control">
              ...
              <option <?php if($mc_status == "Active"){echo 'selected';}?> value="Active"><?php echo "Active";?></option>
              <option <?php if($mc_status == "Inactive"){echo 'selected';}?> value="Inactive"><?php echo "Inactive";?></option>
