...
if(isset($_POST['txtFloor'])){
	...
	$sql = "INSERT INTO `tbl_add_floor`(blockNumber,floor_no,`branch_id`) values('$_POST[blockNumber]','$_POST[txtFloor]','" . $_SESSION['objLogin']['branch_id'] . "')";
	//echo $sql;
	//die();
	...
	$sql = "UPDATE `tbl_add_floor` SET blockNumber='".$_POST['blockNumber']."',`floor_no`='".$_POST['txtFloor']."' WHERE fid='".$_GET['id']."'";
	mysql_query($sql,$link);
      ...
      <form onSubmit="return validateMe();" action="<?php echo $form_url; ?>" method="post" enctype="multipart/form-data">
		...
		<div class="row">
         <div class="form-group col-md-6 col-xs-12">
         	<label><span class="errorStar">*</span><?php echo $_data['add_new_block_title']; ?></label>
         	<select name="blockNumber" class="form-control">
         		<option value=""><?php echo $_data['add_new_select_block'];?></option>
         		<?php 
				$sql=mysql_query("select * from tbl_add_block order by blockId desc");
				while($row=mysql_fetch_assoc($sql))
				{
				
				?>
       	<option value="<?php echo $row['blockId'];?>"<?php if($row['blockId']==@$block_no){ echo "selected"; }?>><?php echo $row['blockNumber'];?></option>
        	<?php } ?>
         	</select>
         </div>
          <div class="form-group col-md-6 col-xs-12">
