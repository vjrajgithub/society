...
include(ROOT_PATH.'language/'.$lang_code_global.'/lang_add_event.php');
...
$form_url = WEB_URL . "event/addevent.php";
$id="";
$hdnid="0";
	...
	mysql_query($sql,$link);
        ...
        if($last_id){
            for($i=0; $i<$totalImages; $i++) { 
                ...
                $date=date('Y-m-d H:i:s');
                $event = 'event';
                $sql = "INSERT INTO t_gallery(event_id,type, image_url, insertDate,updateDate ,c_userid,branch_id) values('$last_id', '$event','$images[$i]','".$date."','".$date."','".(int)$_SESSION['objLogin']['user_id']."','" . $_SESSION['objLogin']['branch_id'] . "')";
                mysql_query($sql,$link);
            }
        }
	mysql_close($link);
	$url = WEB_URL . 'event/eventlist.php?m=add';
	header("Location: $url");
...
//    echo '<pre>';die;
	...
	if(isset($_POST['chkEmpStaus'])){
        ...
        if($_GET['id']){
            echo $sql = "delete from t_gallery WHERE event_id='".$_GET['id']."'";
            //die;
            mysql_query($sql,$link);
                for($i=0; $i<$totalImages; $i++) { 
                date_default_timezone_set("Asia/Kolkata");
                $date=date('Y-m-d H:i:s');
                $event = 'event';
                 $sql = "INSERT INTO t_gallery(event_id,type, image_url, insertDate,updateDate ,c_userid,branch_id) values('$_GET[id]', '$event','$images[$i]','".$date."','".$date."','".(int)$_SESSION['objLogin']['user_id']."','" . $_SESSION['objLogin']['branch_id'] . "')";
                mysql_query($sql,$link);
            }
        }
        
        
	$url = WEB_URL . 'event/eventlist.php?m=up';
	header("Location: $url");
}
...
if(isset($_GET['id']) && $_GET['id'] != ''){
    $queryAllEmployee = "SELECT et.*,gt.event_id,gt.type,gt.image_url FROM t_gallery gt left join t_events et on et.event_id = gt.event_id where  et.event_id = '" . $_GET['id'] . "' group by gt.event_id";
				//die;
...
//			$doc_track = $row['documents'];
		...
		$form_url = WEB_URL . "event/addevent.php?id=".$_GET['id'];
	}
...
<div class="row">
  <div class="col-md-12">
    <div align="right" style="margin-bottom:1%;"> <a class="btn btn-primary" title="" data-toggle="tooltip" href="<?php echo WEB_URL; ?>event/eventlist.php" data-original-title="<?php echo $_data['back_text'];?>"><i class="fa fa-reply"></i></a> </div>
