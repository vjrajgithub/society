...
?>
<?php
include(ROOT_PATH.'language/'.$lang_code_global.'/lang_event_list.php');
$delinfo = 'none';
$addinfo = 'none';
$msg = "";
if(isset($_GET['id']) && $_GET['id'] != '' && $_GET['id'] > 0){
	$sqlx= "DELETE FROM `t_events` WHERE event_id = ".$_GET['id'];
	mysql_query($sqlx,$link); 
        $sqlx= "DELETE FROM `t_gallery` WHERE event_id = ".$_GET['id'];
	mysql_query($sqlx,$link); 
	$delinfo = 'block';
}
...
<div class="row">
  <div class="col-xs-12">
    ...
    <div id="you" class="alert alert-success alert-dismissable" style="display:<?php echo $addinfo; ?>">
      ...
      <?php echo $msg; ?> </div>
    <div align="right" style="margin-bottom:1%;"> <a class="btn btn-primary" data-toggle="tooltip" href="<?php echo WEB_URL; ?>event/addevent.php" data-original-title="<?php echo $_data['add_new_employee_breadcam'];?>"><i class="fa fa-plus"></i></a> <a class="btn btn-primary" data-toggle="tooltip" href="<?php echo WEB_URL; ?>dashboard.php" data-original-title="<?php echo $_data['home_breadcam'];?>"><i class="fa fa-dashboard"></i></a> </div>
...
<!--              <th><?php echo $_data['add_new_form_field_text_2'];?></th>
          ...
          <tbody>
            <?php
                                  $queryAllEmployee = "SELECT et.*,gt.event_id,gt.type,gt.image_url FROM t_gallery gt left join t_events et on et.event_id = gt.event_id where  et.branch_id = " . (int)$_SESSION['objLogin']['branch_id'] . " group by gt.event_id";
				//die;
...
function deleteEmployee(Id){
	...
	if(iAnswer){
		window.location = '<?php echo WEB_URL; ?>event/eventlist.php?id=' + Id;
	}
  }
